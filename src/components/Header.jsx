    ...
    <header className="relative w-full h-[70vh] overflow-hidden font-sans">
      ...
      <nav className="absolute top-6 left-0 right-0 z-40 px-6">
        <div className="max-w-7xl mx-auto bg-white rounded-2xl shadow-xl px-6 py-3 flex items-center relative h-16 md:h-20">
          ...
          {/* LEFT: Desktop Links (Invisible on mobile to give space) */}
              ...
              {navLinks.map((item) => (
                ...
                >
                  {item.hasDropdown ? (
                    ...
                    <NavLink to={item.to} className={({ isActive }) => `text-sm font-bold ${isActive ? "text-amber-500" : "text-slate-700 hover:text-amber-500"}`}>
                      {item.label}
                    </NavLink>
                  )}
                  ...
                  {item.hasDropdown && isDropdownOpen && (
                    <div className="absolute top-full left-0 w-48 bg-white border border-slate-100 rounded-xl shadow-2xl py-2 z-[60]">
                      {item.subLinks.map((sub) => (
                        <NavLink key={sub.to} to={sub.to} className="block px-4 py-2.5 text-xs font-black uppercase text-slate-700 hover:bg-amber-50 hover:text-amber-600">
                          {sub.label}
                        </NavLink>
                      ))}
                    </div>
          ...
          <div className="absolute left-1/2 -translate-x-1/2 -bottom-6 z-50">
            <div className="bg-white rounded-b-[70%] flex items-center justify-center w-36 md:w-44 h-20 md:h-24  border-t-0">
              <NavLink to="/">
                <img src="/src/assets/Logo.png" alt="Travlla Logo" className="h-14 md:h-18 w-auto object-contain" />
              </NavLink>
            </div>
          </div>

          {/* RIGHT: Socials & Hamburger (Pinned to Far Right) */}
          <div className="flex-1 flex justify-end items-center ml-auto">
            ...
            <div className="hidden md:flex items-center gap-4 text-slate-700 mr-2">
              <NavLink to="#" className="hover:text-amber-500 transition-colors">
                <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="3"/><circle cx="17.5" cy="6.5" r="0.5" fill="currentColor"/></svg>
              </NavLink>
              <NavLink to="#" className="hover:text-amber-500 transition-colors">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M22 5.92c-.62.28-1.28.47-1.98.55a3.47 3.47 0 001.52-1.92 6.93 6.93 0 01-2.2.84 3.45 3.45 0 00-5.88 3.14A9.79 9.79 0 013 4.87a3.45 3.45 0 001.07 4.6c-.5-.02-.98-.15-1.4-.38v.04a3.45 3.45 0 002.76 3.38c-.48.13-.99.13-1.48.05a3.46 3.46 0 003.22 2.4A6.92 6.92 0 012 18.58a9.76 9.76 0 005.29 1.55c6.34 0 9.81-5.25 9.81-9.8v-.45c.67-.49 1.26-1.1 1.72-1.79-.62.28-1.29.47-1.98.55z" /></svg>
              </NavLink>
            </div>

            {/* Hamburger - Only visible on Mobile, forced to the right wall */}
            <button
                  ...
                  {item.hasDropdown ? (
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col gap-4 pl-4 border-l-2 border-amber-500">
                        {item.subLinks.map(sub => (
                          <NavLink key={sub.to} to={sub.to} onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-bold text-slate-800 hover:text-amber-500">
                            {sub.label}
                          </NavLink>
                        ))}
                      </div>
                    ...
                    <NavLink to={item.to} onClick={() => setIsMobileMenuOpen(false)} className={({isActive}) => `text-2xl font-black block ${isActive ? "text-amber-500" : "text-slate-900 hover:text-amber-500"}`}>
                      {item.label}
                    </NavLink>
                  )}
                </li>
      ...
      <div className="relative z-20 h-full flex items-center px-6">
        <div className="max-w-7xl mx-auto w-full">
          {variant === "tour" ? (
            <div className="pt-10">
              <h1 className="text-4xl md:text-7xl font-extrabold text-white leading-tight drop-shadow-lg">{title}</h1>
              {subtitle && <p className="mt-4 text-white/90 text-lg md:text-2xl max-w-2xl">{subtitle}</p>}
            </div>
          ) : (
