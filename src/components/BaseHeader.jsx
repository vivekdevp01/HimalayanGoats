...
const logosrc = '/assets/TRIPSHALA.png';
  ...
  return (
    /* Reduced padding and set sticky with a clean white bg */
    <nav className="sticky top-0 left-0 right-0 z-[100] px-6 bg-white shadow-sm border-b border-slate-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between h-16 md:h-20 relative">
        ...
        {/* LOGO - Scaled down for the smaller navbar height */}
        <div className="flex-shrink-0 relative z-[110]">
          <NavLink to="/">
            <img
              src={logosrc}
              alt="Tripshalla"
              /* h-12 to h-16 is perfect for a slim 20-unit navbar */
              className="h-10 md:h-14 w-auto object-contain transition-transform hover:scale-105"
            />
          </NavLink>
        </div>

        {/* DESKTOP LINKS - Using tight font tracking for a modern look */}
          ...
          {navLinks.map((l) => (
            ...
            >
              ...
              {l.hasDropdown && isDropdownOpen && (
                <div 
                  className="absolute top-[100%] left-0 w-48 z-[120] pt-2"
                  ...
                  <div className="bg-white rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] border border-slate-100 py-2 overflow-hidden">
                    {l.subLinks.map((sub) => (
                      <NavLink
                        key={sub.to}
                        to={sub.to}
                        className="block px-5 py-2.5 text-[11px] font-black tracking-widest text-[#1A1D1F] hover:bg-[#F7A325] hover:text-white transition-all duration-200 uppercase"
                      >
                        {sub.label}
                      </NavLink>
                    ))}
        ...
        <div className="flex items-center gap-4 relative z-[110]">
          <button
            ...
            className="lg:hidden text-[#1A1D1F] p-2 focus:outline-none"
          >
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[280px] bg-white text-slate-900 z-[150] shadow-2xl transform transition-transform duration-500 ease-in-out lg:hidden ${
              ...
              <li key={l.key} className="border-b border-slate-50 pb-3">
                {l.hasDropdown ? (
                  ...
                  <NavLink
                    ...
                    onClick={() => setIsMenuOpen(false)}
                    className={({ isActive }) =>
                      `block ${isActive ? "text-[#F7A325]" : "text-slate-900"}`
                    }
                  >
                    {l.label}
                  </NavLink>
